...
- [x] **21. Худалдагчийн статистик (Shop Stats)** — `products.view_count` (schema v23); барааны хуудсанд эзэн биш үзэлт нэмэгдүүлнэ (`incrementProductView`); `getSellerShopStats` (нийт үзэлт/дуртай/зарагдсан/хөрвүүлэлт + бараа тус бүрийн задаргаа). Dashboard-д "📊 Дэлгүүрийн статистик" — 4 summary card + бараа тус бүрийн хүснэгт (`ShopStats` компонент). `products-db.test.ts`-д 2 тест. tsc + build + 36 тест цэвэр. — **M** ✓ ДУУССАН
- [x] **22. Бүтээгдэхүүний хувилбар + захиалгаар хийх** — schema v24: `product_options` + `product_option_values` (хүртэл 3 бүлэг), `products.personalization_prompt`, `order_items.variant`+`personalization`. Шинэ бараа форм дээр зохионо (засах хуудас байхгүй дизайнд нийцүүлэв); барааны хуудсанд option `<select>`-үүд + захиалгын текст талбар (бөглөтөл сагсанд нэмэхийг хориглоно); cart-ийг composite line key (`cartLineKey` = product+variant+personalization) рүү refactor (ижил бараа өөр хувилбараар тусдаа мөр); checkout JSON + `createOrdersFromCart` → order_item-д snapshot; сагс/худалдан авагч/борлуулагчийн захиалгад харагдана. `orders-db.test.ts` +2, `products-db.test.ts` +2. tsc + build + 40 тест цэвэр. End-to-end (option + personalization render) баталгаажсан. — **M-L** ✓ ДУУССАН
